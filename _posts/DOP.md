...
CPU Cache 和内存之间传输数据的最小单位是缓存行（一般为 64 字节）。
...
- 第二个参数是 \_\_m128 类型，一般配合 SIMD 指令使用
...
答：不是，只要对访问的区域进行分块，例如让每一块的大小都相当于装满 Cache 的容量（例如 L1 Cache 有 32KB），那么在这个块内的任何次序的访问都是可以命中 Cache 的
...
> 例如，两个 int32 可以打包成一个 int64，四个 int32 可以打包成一个**m128，八个 int32 可以打包为一个**m256（需支持 AVX 指令集）
...
- SSE 指令集：128 位操作（ 4×32bits 或者 2×64bits ）
- AVX 指令集：256 位操作（ 8×32bits 或者 4×64bits）
...
> ```
>
> `__restrict`  保证 a 和 b 不会有数据依赖，可让编译器大胆执行 SIMD 优化
>
> ```cpp
...
在 ISPC 语法里，只需简单的写上  `foreach(i = 0 ... N)` ，IPSC 编译器编译时会为其编译成图中右侧的行为，即一次循环并行处理 M 个元素，实际循环 N/M 次。
...
SIMD 寄存器读取变量一般都是一次性读取连续若干个（在图中为 4 个）变量，这种行为叫做矢量读取（Vector Load）。
...
> 注意是对象数组，而不是指针数组。如果是指针数组的话，这对 CPU 缓存命中没有意义（因为要通过指针跳转到不相邻的内存）
...
- [5] [浅谈 Unity ECS（一）Uniy ECS 基础概念介绍：面向未来的 ECS](https://zhuanlan.zhihu.com/p/59879279)
- [6] [Unity3D DOTS | ECS concepts](https://docs.unity3d.com/Packages/com.unity.entities@0.50/manual/ecs_core.html)
- [7] [使用英特尔 ® ISPC 简化 SIMD 开发 | 英特尔 ® 软件](https://software.intel.com/zh-cn/videos/simplifying-simd-development-with-ispc)
